...
Также если у Вас имеется API-ключ, то Вы сможете войти в ваш аккаунт
через API и создавать ивенты с помощью нашего API.

Для использования API-ключа укажите в headers в ключе Authorization ```Api-Key <ВАШ-API-ключ>```

Для создания ивентов войдите через API на ```/api/v1/login/```, вам вернётся токен.
Для использования токена укажите в headers в ключе Authorization ```Token <ВАШ-ТОКЕН>```.
С помощью токена вы сможете создавать ивенты и просматривать данные одновременно!
...
```/api/v1/register/``` POST-запрос, в body запроса укажите ник и заполните поля с паролями. Регистрирует новых
пользователей

```/api/v1/login/``` POST-запрос, в body запроса укажите ник и заполните и пароль от существующего аккаунта.
Выдаёт токен для создания ивентов от имени пользователя
...
```/api/v1/events/``` GET. Возвращает события, которые доступны для просмотра всем. Для POST создаёт новое событие,
если в header "Authorization" указан Token
